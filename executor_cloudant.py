...
from pywren_research.libs.cloudant.cloudant_client import CloudantClient
    ...
    def __init__(self, config=None, runtime=None, runtime_memory=None, compute_backend=None,
        ...
        if eval(os.environ.get('PYWREN_EVENT_SOURCING', 'False')):
            ...
            if os.environ.get('PYWREN_FIRST_EXEC') == 'False':
                to = time.time()
                cloudant_client = CloudantClient(**tf_config['cloudant'])
            ...
            for subject in records:
                ...
                if executor_id == self.executor_id:
                    for message in records[subject]:
                        ...
                        self.event_sourcing_jobs[job_id].append(event)
    ...
    def call_async(self, func, data, extra_env=None, runtime_memory=None,
        ...
        if self.event_sourcing and not already_invoked:
            self.ep.add_trigger(
                ...
                context={'subject': 'pywren_coordinator',
                         'function_args': {'execution_id': self.executor_id.split('/')[0], 'start_time': os.environ.get('START_TIME')},
